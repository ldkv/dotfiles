...
DOTFILES_PATH = Path(__file__).parent.parent.resolve()
...
class Config:
    ...
    @classmethod
    def get_dotfiles_path(cls) -> Path:
        return DOTFILES_PATH
        dotfiles_path_from_env = get_env_var("DOTFILES_PATH")
        if dotfiles_path_from_env:
            print(f"Found DOTFILES_PATH from env: {dotfiles_path_from_env}")
            return Path(dotfiles_path_from_env).resolve()
    ...
    def path(cls) -> Path:
        ...
        return DOTFILES_PATH / "scripts" / file_name
